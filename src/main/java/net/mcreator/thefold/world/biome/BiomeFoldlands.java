...
import net.mcreator.thefold.block.BlockUnboundWoof;
import net.mcreator.thefold.block.BlockUnboundLeaves;
...
public class BiomeFoldlands extends ElementsTheFold.ModElement {
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockFoldGrass.block.getDefaultState().getBlock() || ground == BlockFoldDurt.block.getDefaultState().getBlock())
							&& (ground2 == BlockFoldGrass.block.getDefaultState().getBlock()
									|| ground2 == BlockFoldDurt.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockFoldDurt.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockUnboundLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockUnboundLeaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockUnboundLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockUnboundWoof.block.getDefaultState());
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockUnboundWoof.block.getDefaultState().getBlock()
					|| blockType == BlockUnboundLeaves.block.getDefaultState().getBlock()
					|| blockType == BlockFoldGrass.block.getDefaultState().getBlock()
					|| blockType == BlockFoldDurt.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockFoldDurt.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockFoldDurt.block.getDefaultState());
		}
